...
import { Apple, Calendar, Dumbbell } from 'lucide-react';
        ...
        <div className='relative backdrop-blur-sm border border-border p-6'>
          ...
          <div className='flex flex-wrap gap-2'>
            {allPlans.map((plan) => (
              ...
              className={`text-foreground border hover:text-white ${selectedPlanId === plan._id ? "bg-primary/20 text-primary border-primary" : "bg-transparent border-border hover:border-primary/50"}`}
              >
                {plan.name}
                {plan.isActive && (
                  <span className='ml-2 bg-green-500/20 text-green-500 text-xs px-3 py-0.5 rounded'>
                    ACTIVE
                  </span>
                )}
              </Button>
                    ...
                    <Accordion type='multiple' className='space-y-4'>
                      {currentPlan.workoutPlan.exercises.map((exerciseDay, index) => (
                        ...
                        >
                          ...
                          <AccordionContent className='pbb-4 px-4'>
