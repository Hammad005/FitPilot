...
const ProfileHeader = ({ user }: { user: UserResource | null | undefined}) => {
    if (!user) return null;
    return (
        <div className="mb-10 relative backdrop-blur-sm border border-border p-8">
            <CornerElements />
...
<div className="flex-1">
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
        ...
        <div className="flex items-center bg-cyber-terminal-bg backdrop-blur-sm border border-b rounded px-3 py-1">
            <div className="size-2 rounded-full bg-primary animate-pulse mr-2"/>
            <p className="text-xs font-mono text-primary">USER ACTIVE</p>
        </div>
    </div>
    <div className="h-px w-full bg-gradient-to-r from-primary via-secondary to-primary opacity-5- my-2"/>
